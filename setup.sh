...
if echo "$XDG_CURRENT_DESKTOP" | grep -iq "GNOME"; then
    ...
    echo "Skrypt z domyślnym zachowaniem dla GNOME/KDE może nie zadziałać w pełni poprawnie."
...
echo "Instaluję podstawowe narzędzia (base-devel, git, flatpak, itp.)..."
sudo pacman -S --needed base-devel git wget cmake flatpak ntfs-3g --noconfirm
...
if [ "$DE" = "GNOME" ]; then
    echo "Instaluję pakiety dla GNOME..."
...
# 6. INSTALACJA ROZSZERZEŃ I MOTYWÓW
if [ "$DE" = "GNOME" ]; then
...
# 10. FINALIZACJA WYGLĄDU (Kursor, Ikony, Tapeta)
...
if [ "$DE" = "GNOME" ]; then
    ...
    # Komenda dla Plazmy do zmiany kursora
